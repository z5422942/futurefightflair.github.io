...
flair.selectChoice = function (hero_id, key) {
	...
	flair.by_id[hero_id].hero_abilities1 = flair.by_id[hero_id].hero_attributeop.replace(/.*?(\#(Agent|Agility|Chaos Magic|Chill|Cold Blooded|Command|Durability|Energy Projection|Fast Movement|Flame|Healing|Heightened Senses|Leadership|Machine|Magic|Mind|Mind Resist|Phoenix Force|Poison|Pure Evil|Shock|Spider-Sense|Strong|Symbiote|Time Freezing Immunity|Weapons Master)*\#).*?/gi, "$1").replace(/#Agent#/g, " Agent,").replace(/#Agility#/g, " Agility,").replace(/#Chaos Magic#/g, " Chaos Magic,").replace(/#Chill#/g, " Chill,").replace(/#Cold Blooded#/g, " Cold Blooded,").replace(/#Command#/g, " Command,").replace(/#Durability#/g, " Durability,").replace(/#Energy Projection#/g, " Energy Projection,").replace(/#Fast Movement#/g, " Fast Movement,").replace(/#Flame#/g, " Flame,").replace(/#Healing#/g, " Healing,").replace(/#Heightened Senses#/g, " Heightened Senses,").replace(/#Leadership#/g, " Leadership").replace(/#Machine#/g, " Machine,").replace(/#Magic#/g, " Magic,").replace(/#Mind#/g, " Mind,").replace(/#Mind Resist#/g, " Mind Resist").replace(/#Phoenix Force#/g, " Phoenix Force,").replace(/#Poison#/g, " Poison,").replace(/#Pure Evil#/g, " Pure Evil,").replace(/#Shock#/g, " Shock").replace(/#Spider-Sense#/g, " Spider-Sens,e").replace(/#Strong#/g, " Strong,").replace(/#Symbiote#/g, " Symbiote,").replace(/#Time Freezing Immunity#/g, " Time Freezing Immunity,").replace(/#Weapons Master#/g, " Weapons Master,");
	flair.by_id[hero_id].hero_abilities = flair.by_id[hero_id].hero_abilities1.replace(/ *\#[^)]*\# */g, "");
...
flair.loadChoices = function () {
	...
	for (var hero_id in flair.by_id) {
		if (flair.by_id.hasOwnProperty(hero_id)) {
			...
			flair_choice.setAttribute('title', data.hero_name);
